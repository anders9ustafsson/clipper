...
namespace ClipperLib
{
  ...
  public class Clipper : ClipperBase
  {
      ...
      private void AddJoin(OutPt Op1, OutPt Op2, FPoint OffPt)
      {
        Join j = new Join();
        j.OutPt1 = Op1;
        j.OutPt2 = Op2;
      ...
      private void AddGhostJoin(OutPt Op, FPoint OffPt)
      {
        Join j = new Join();
        j.OutPt1 = Op;
        j.OffPt = OffPt;
...
#if use_xyz
      internal void SetZ(ref FPoint pt, TEdge e)
      {
        pt.Z = 0;
      ...
      private void AddLocalMaxPoly(TEdge e1, TEdge e2, FPoint pt)
      {
          AddOutPt(e1, pt);
      ...
      private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, FPoint pt)
      {
        OutPt result;
        TEdge e, prevE;
      ...
      private OutPt AddOutPt(TEdge e, FPoint pt)
      {
      ...
      internal void SwapPoints(ref FPoint pt1, ref FPoint pt2)
      {
          FPoint tmp = new FPoint(pt1);
          pt1 = pt2;
          pt2 = tmp;
      }
      //------------------------------------------------------------------------------
      ...
      private bool HorzSegmentsOverlap(
        FPoint Pt1a, FPoint Pt1b, FPoint Pt2a, FPoint Pt2b)
      {
      ...
      private OutPt InsertPolyPtBetween(OutPt p1, OutPt p2, FPoint pt)
      {
      ...
      private double GetDx(FPoint pt1, FPoint pt2)
      {
      ...
      private void IntersectEdges(TEdge e1, TEdge e2, FPoint pt, bool protect = false)
      {
  ...
  //assumes that e1 will be to the Right of e2 ABOVE the intersection
        ...
        for (;;)
        {
          ...
          while(e != null)
          {
              ...
              (dir == Direction.dRightToLeft && e.Curr.X >= horzLeft))
            {
              ...
              {
                FPoint Pt = new FPoint(e.Curr.X, horzEdge.Curr.Y);
      ...
      private bool ProcessIntersections(double botY, double topY)
      {
      ...
      private void BuildIntersectList(double botY, double topY)
      {
      ...
      private bool IntersectPoint(TEdge edge1, TEdge edge2, out FPoint ip)
      {
        ip = new FPoint();
        double b1, b2;
    ...
    //------------------------------------------------------------------------------
      ...
      private static double DistanceSqrd(FPoint pt1, FPoint pt2)
      {
      ...
      private static double DistanceFromLineSqrd(FPoint pt, FPoint ln1, FPoint ln2)
      {
      ...
      private static bool SlopesNearCollinear(FPoint pt1, 
          FPoint pt2, FPoint pt3, double distSqrd)
      {
        return DistanceFromLineSqrd(pt2, pt1, pt3) < distSqrd;
      }
      //------------------------------------------------------------------------------

      private static bool PointsAreClose(FPoint pt1, FPoint pt2, double distSqrd)
      {
          double dx = (double)pt1.X - pt2.X;
          double dy = (double)pt1.Y - pt2.Y;
      ...
      internal static Paths Minkowski(Path poly, Path path, bool IsSum, bool IsClosed)
      {
        ...
        if (IsSum)
          for (int i = 0; i < pathCnt; i++)
          {
            ...
            foreach (FPoint ip in poly)
              p.Add(new FPoint(path[i].X + ip.X, path[i].Y + ip.Y));
            result.Add(p);
          }
        else
          for (int i = 0; i < pathCnt; i++)
  ...
  public class ClipperOffset
  {
    ...
    private List<FPoint> m_normals = new List<FPoint>();
    ...
    {
      return value < 0 ? (double)(value - 0.5) : (double)(value + 0.5);
    }
    //------------------------------------------------------------------------------
    ...
    internal static FPoint GetUnitNormal(FPoint pt1, FPoint pt2)
    {
      double dx = (pt2.X - pt1.X);
      double dy = (pt2.Y - pt1.Y);
      if ((dx == 0) && (dy == 0)) return new FPoint();

      double f = 1 * 1.0 / Math.Sqrt(dx * dx + dy * dy);
      dx *= f;
      dy *= f;

      return new FPoint(dy, -dx);
    }
    //------------------------------------------------------------------------------
    ...
    private void DoOffset(double delta)
    {
      ...
      for (int i = 0; i < m_polyNodes.ChildCount; i++)
      {
        ...
        if (node.m_endtype == EndType.etClosedLine || 
          ...
          m_normals.Add(new FPoint(m_normals[len - 2]));
        ...
        else if (node.m_endtype == EndType.etClosedLine)
        {
          ...
          for (int j = len - 1; j > 0; j--)
            m_normals[j] = new FPoint(-m_normals[j - 1].X, -m_normals[j - 1].Y);
          m_normals[0] = new FPoint(-n.X, -n.Y);
          k = 0;
          for (int j = len - 1; j >= 0; j--)
            OffsetPoint(j, ref k, node.m_jointype);
          m_destPolys.Add(m_destPoly);
        ...
        else
        {
          ...
          else
          {
            ...
            k = len - 2;
            m_sinA = 0;
            m_normals[j] = new FPoint(-m_normals[j].X, -m_normals[j].Y);
          ...
          m_normals[0] = new FPoint(-m_normals[1].X, -m_normals[1].Y);
    ...
    public void Execute(ref Paths solution, double delta)
    {
      ...
      {
        FRect r = Clipper.GetBounds(m_destPolys);
    ...
    internal void DoRound(int j, int k)
    {
      ...
      {
        m_destPoly.Add(new FPoint(m_srcPoly[j].X + X * m_delta,
