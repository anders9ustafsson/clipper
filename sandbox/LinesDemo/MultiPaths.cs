...
namespace ClipperMultiPathsLib
{
  ...
  public class MultiPaths
  {
    ...
    public static void ClipCallback(IntPoint vert1, IntPoint vert2, ref IntPoint intersectPt) 
    {
      ...
      //The 'Z' values generated by MultiPaths are always even (see MakeZ) so that the odd 
      //values generated here flag to Reconstruct() that the vertex was at an intersection ...
      if (Math.Abs(vert1.Z - vert2.Z) > 2)
      {
        //we've hit the end of a segment to get here (note stripping of any overlap flag too)
        if ((UInt32)vert1.Z == 0)
    ...
    public MultiPath Reconstruct(Path flatPath)
    {
      ...
      if (mp1.IsClosed && i == 0)
      {
        ...
        else if (IsAdjacent(flatPath[0].Z, flatPath[highI].Z))
        {
          ...
          if (IsOverlap(flatPath[highI - j].Z, flatPath[0].Z)) p.Add(flatPath[highI - j]); //nb: don't j++ this one!
      ...
      while (i <= highI)
      {
        ...
        //reuse mp2 to get the reconstructed MultiPath ...
        //finally, something to reconstruct /////////////////////////// 
        mp2 = mp1.Reconstruct(p);
        //////////////////////////////////////////////////////////////
        //and append each segment in mp2 to result ...
        foreach (MultiPathSegment mps2 in mp2)
        {
          MultiPathSegment mps = result.NewMultiPathSegment(mps2.curvetype, null);
          mps2.CopyTo(mps);
        }
  ...
  public class MultiPath
  {
    ...
    public MultiPath Reconstruct(Path flatPath) 
    {
      ...
      int seg1 = (UInt16)(flatPath[1].Z >> 16) & 0xFFFF;           //likewise avoid 
      int seg2 = (UInt16)(flatPath[highI - 1].Z >> 16) & 0xFFFF;   //'overlap' vertices
      ...
      mps.Reconstruct(flatPath[0], flatPath[highI], result);
      
      if (reversed)
      {
        foreach (MultiPathSegment mpsr in result.segments)
          mpsr.Reverse();
      ...
      int cnt = 0;
      foreach (MultiPathSegment mpsr2 in result) cnt += mpsr2.Count;
  ...
  public abstract class MultiPathSegment
  {
    ...
    internal void CopyTo(MultiPathSegment mps)
    {
      foreach (IntPoint ip in ctrls) mps.ctrls.Add(ip);
    }
    //------------------------------------------------------------------------------

    internal void CopyTo(Path path)
    {
      foreach (IntPoint ip in ctrls) path.Add(ip);
    }
    //------------------------------------------------------------------------------
    ...
    internal abstract void Reconstruct(IntPoint startPt, IntPoint endPt, MultiPath mp);
  ...
  public class MpsLine : MultiPathSegment
  {
    ...
    internal override void Reconstruct(IntPoint startPt, IntPoint endPt, MultiPath mp)
    {
      ...
      if (startPt.Z == 0)
      {
        ...
        startIdx = ((UInt16)startPt.Z - flattenOffset) / 2;
      ...
      if (endPt.Z == 0)
      {
        ...
        endIdx = ((UInt16)endPt.Z + 1 - flattenOffset) / 2;

      if (endIdx >= ctrls.Count) endIdx = ctrls.Count - 1;
      linePath.Add(startPt);
      for (int k = startIdx + 1; k < endIdx; k++) linePath.Add(ctrls[k]);
      linePath.Add(endPt);
      int mpCnt = mp.Count;
      if (mpCnt > 0 && mp[mpCnt - 1].curvetype == CurveType.Line)
        foreach (IntPoint ip in ctrls) mp[mpCnt - 1].Add(ip);
      else
        mp.NewMultiPathSegment(CurveType.Line, linePath);
    }
    //---------------------------------------------------------------------------
  ...
  public class MpsEllipticalArc : MultiPathSegment
  {
    ...
    internal override void Flatten()
    {
      ...
      this.radii.X = Globals.Distance(origin, axis1);
      this.radii.Y = Globals.Distance(origin, axis2);
  ...
  public abstract class MpsBezier : MultiPathSegment
  {
    ...
    internal override void Reconstruct(IntPoint startPt, IntPoint endPt, MultiPath mp)
    {
      ...
      if (!IsValid() || BezSegList.Count == 0)
      {
        Path path = new Path(2);
        path.Add(startPt);
        path.Add(endPt);
        mp.NewMultiPathSegment(CurveType.Line, path);
        return;
      };
      ...
      //todo ...
      //if (intlist.Count * this.BezSegList[0].Ctrls.Length >= (endIdx - startIdx))
  ...
  public class Globals
  {
      ...
      double radius, bool IsClockwise)
    {
      ...
      origin.X = mp.X + sign * opp * (dp1.Y - dp2.Y) / adjMul2;
      origin.Y = mp.Y + sign * opp * (dp2.X - dp1.X) / adjMul2;
      return origin;
    }
    //---------------------------------------------------------------------------
      ...
      out DoublePoint origin, out DoublePoint radii, out double angle_radians)
    {
      ...
      radii = new DoublePoint(Distance(axis1a, origin), 0);
      pt = RotatePoint(origin, pt, -angle_radians); //ie rotate pt so it's axis aligned
      radii.Y = GetEllipticalRadiusY(radii.X, origin, pt);
      return radii.Y > 0;
    }
    //------------------------------------------------------------------------------
      ...
      DoublePoint radii, double angle_radians, bool largeArc, bool sweep)
    {
      ...
      if (radii.X < 0.0) radii.X = -radii.X;
      if (radii.Y < 0.0) radii.Y = -radii.Y;
      ...
      if (d > 1)
      {
        ...
        radii.X = sqrtD * radii.X;
        radii.Y = sqrtD * radii.Y;
