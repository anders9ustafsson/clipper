...
DoublePoint GetUnitNormal(const FPoint &pt1, const FPoint &pt2)
{
  ...
  double Delta, JoinType jointype, EndType endtype, double limit): m_p(in_polys)
{
  ...
  for (m_i = 0; m_i < m_p.size(); m_i++)
  {
      ...
      if (len == 1)
      {
        ...
        {
          double X = 1.0, Y = 0.0;
          for (double j = 1; j <= m_Steps360 * 2 * pi; j++)
          {
            AddPoint(FPoint(
              m_p[m_i][0].X + X * Delta,
              m_p[m_i][0].Y + Y * Delta));
        ...
        } else
        {
          ...
          for (int j = 0; j < 4; ++j)
          {
            AddPoint(FPoint(m_p[m_i][0].X + X * Delta,
              m_p[m_i][0].Y + Y * Delta));
            if (X < 0) X = 1;
            else if (Y < 0) Y = 1;
            else X = -1;
      ...
      else //is open polyline
      {
        ...
        {
          m_j = len - 1;
          pt1 = FPoint(m_p[m_i][m_j].X + normals[m_j].X * m_delta, 
        ...
        if (endtype == etButt) 
        {
          pt1 = FPoint(m_p[m_i][0].X - normals[0].X * m_delta, 
            m_p[m_i][0].Y - normals[0].Y * m_delta);
          AddPoint(pt1);
          pt1 = FPoint(m_p[m_i][0].X + normals[0].X * m_delta, 
            m_p[m_i][0].Y + normals[0].Y * m_delta);
          AddPoint(pt1);
...
void OffsetPoint(JoinType jointype)
{
  ...
  if (m_sinA * m_delta < 0)
  {
    AddPoint(FPoint(m_p[m_i][m_j].X + normals[m_k].X * m_delta,
      m_p[m_i][m_j].Y + normals[m_k].Y * m_delta));
    AddPoint(m_p[m_i][m_j]);
    AddPoint(FPoint(m_p[m_i][m_j].X + normals[m_j].X * m_delta,
      m_p[m_i][m_j].Y + normals[m_j].Y * m_delta));
  }
  else
    switch (jointype)
...
{
    double q = m_delta / r;
    AddPoint(FPoint(m_p[m_i][m_j].X + (normals[m_k].X + normals[m_j].X) * q,
...
void StripDupsAndGetBotPt(const Path& in_path, Path& out_path, bool closed, FPoint* botPt)
{
  botPt = 0;
  ...
  double delta, JoinType jointype, EndType endtype, double limit)
{
  ...
  {
    StripDupsAndGetBotPt(in_polys[i], inPolys[i], endtype == etClosed, pt);
    if (botPt) 
...
DoublePoint ClosestPointOnLine(const FPoint& Pt, const FPoint& linePt1, const FPoint& linePt2)
{
